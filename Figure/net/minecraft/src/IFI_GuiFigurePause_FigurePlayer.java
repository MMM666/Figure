...
public class IFI_GuiFigurePause_FigurePlayer extends IFI_GuiFigurePause {
    ...
    public void initGui()
    {
    	super.initGui();
        try {
        	String[] str1 = (String[])ModLoader.getPrivateValue(RenderPlayer.class, null, 3);
        ...
        for (int i = 0; i < armorsSelection.length; i++) {
        	for (int j = 0; j < armorsName.length; j++) {
        		if (efplayer.armorPrefix[i] != null && armorsName[j].compareTo(efplayer.armorPrefix[i]) == 0) {
        			armorsSelection[i] = j;
        			break;
        		}
        	}
        ...
        controlList.add(new GuiButton(153, width / 2 - 140, height / 6 +  0 + 12, 80, 20, getArmorName(armorsSelection[3])));
        controlList.add(new GuiButton(152, width / 2 - 140, height / 6 + 24 + 12, 80, 20, getArmorName(armorsSelection[2])));
        controlList.add(new GuiButton(151, width / 2 - 140, height / 6 + 48 + 12, 80, 20, getArmorName(armorsSelection[1])));
        controlList.add(new GuiButton(150, width / 2 - 140, height / 6 + 72 + 12, 80, 20, getArmorName(armorsSelection[0])));
        controlList.add(new GuiButton(100, width / 2 - 140, height / 6 + 96 + 12, 80, 20, getSkinMode()));
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        case 153:
        	...
        	if (++armorsSelection[index] == armorsName.length) {
